<?php
	...
	if (!function_exists('dbInit')) {
		function dbInit()
		{
			echo "hello";
			$mysqli = new mysqli("127.0.0.1", "imagine");
			
			if ($mysqli->connect_errno){
				echo "Failed to connect to database: ({$mysqli->connect_errno}): {$mysqli->connect_error}";
			}else{
			echo "got it!";
			$mysqli->select_db("imagine");
			}
			return $mysqli;
		}
	}
	...
	if (!function_exists('listProvocations')) {
		function listProvocations($ppLim = 20)
		{
			$mysqli = dbInit();
			
			$resultsArr = array();
			
			if ($stmt = $mysqli->prepare("SELECT id, title, type, author FROM imagine LIMIT ?"))
			{
				$stmt->bind_param('i', $ppLim);
				$stmt->execute();
				$stmt->store_result();
	...
	// 1 = image
	// 2 = text
	if (!function_exists('insertProvocation')) {
	...
	if (!function_exists('loadProvocation')) {
	...
	if (!function_exists('loadRandomProvocation')) {
	...
	if (!function_exists('loadRandomProvocationJSON')) {
